...
from hparser cimport HTTP_REQUEST, HTTP_RESPONSE, http_parser_type, flags, http_parser_url_fields, http_parser_url, http_parser_version, http_parser, http_parser_settings, http_parser_init, http_parser_execute, http_should_keep_alive, http_parser_pause, http_body_is_final, HTTP_PARSER_ERRNO, http_errno_name, http_errno_description, http_parser_url, http_parser_url_init, http_parser_parse_url, UF_SCHEMA, UF_HOST, UF_PORT, UF_PATH, UF_QUERY, UF_FRAGMENT, UF_USERINFO, UF_MAX
...
cdef int on_message_begin(http_parser *parser) except -1:
...
cdef int on_status(http_parser *parser, char *data, size_t length) except -1:
    cdef object parser_data = <object> parser.data
...
cdef int on_chunk_header(http_parser *parser) except -1:
    cdef object parser_data = <object> parser.data
    parser_data.delegate.on_chunk_header()
    return 0

cdef int on_chunk_complete(http_parser *parser) except -1:
    cdef object parser_data = <object> parser.data
...
cdef class HttpEventParser(object):
    ...
    def __cinit__(self, object delegate, http_parser_type parser_type):
        # init parser
    ...
    cdef int _execute(self, char *data, size_t length):
        ...
        nparsed = http_parser_execute(self._parser, &self._settings, data, length)

        if nparsed != length:
            errno = HTTP_PARSER_ERRNO(self._parser)
            name = http_errno_name(errno)
            desc = http_errno_description(errno)
...
cdef class HttpUrlParser(object):
    cdef http_parser_url *_parser
    cdef object data

    def __cinit__(self):
        # init parser
        self._parser = <http_parser_url *> malloc(sizeof(http_parser_url))
        http_parser_url_init(self._parser)
    ...
    cdef object _parse(self, char *url, size_t length, bool is_connect):
        ...
        rv = http_parser_parse_url(url, length, is_connect, self._parser)
        if rv != 0:
            raise Exception('URL Parser gave error: {rv}'.format(rv=rv))

        ret = []
        for i in xrange(UF_MAX):
            if i == UF_PORT:
